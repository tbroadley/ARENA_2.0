...
def forward_and_back(a: Arr, b: Arr, c: Arr) -> Tuple[Arr, Arr, Arr]:
    ...
    # Backward
    grad_g = np.array(1.0) # dL/dg = dg/dg = 1
    grad_f = log_back(grad_out=grad_g, out=g, x=f)
...
def log_forward(x: Tensor) -> Tensor:
    ...
    if grad_tracking_enabled and (x.requires_grad or x.recipe is not None):
        ...
        result = Tensor(np.log(x.array), requires_grad=True)
...
def multiply_forward(a: Union[Tensor, int], b: Union[Tensor, int]) -> Tensor:
    ...
    a_to_multiply = a.array if isinstance(a, Tensor) else a
    b_to_multiply = b.array if isinstance(b, Tensor) else b
    out = np.multiply(a_to_multiply, b_to_multiply)
    ...
    a_requires_grad = isinstance(a, Tensor) and (a.requires_grad or a.recipe is not None)
    b_requires_grad = isinstance(b, Tensor) and (b.requires_grad or b.recipe is not None)

    if grad_tracking_enabled and (a_requires_grad or b_requires_grad):
        recipe = Recipe(
            func=np.multiply,
            # We should handle the fact that a or b could be a scalar
            args=(a_to_multiply, b_to_multiply),
            kwargs={},
            parents=parents,
        )
...
def wrap_forward_fn(numpy_func: Callable, is_differentiable=True) -> Callable:
    ...
    def tensor_func(*args: Any, **kwargs: Any) -> Tensor:
        ...
        requires_grad = grad_tracking_enabled and is_differentiable and any(
            isinstance(arg, Tensor) and (arg.requires_grad or arg.recipe is not None)
            for arg in args
        )
        
        numpy_func_args = [a.array if isinstance(a, Tensor) else a for a in args]
        out = numpy_func(*numpy_func_args, **kwargs)
        ...
        if requires_grad:
            ...
            recipe = Recipe(func=numpy_func, args=numpy_func_args, kwargs=kwargs, parents=parents)
            result.recipe = recipe
...
def topological_sort(node: Node, get_children: Callable) -> List[Node]:
    ...
    # Marks nodes in the path from the root to the current node. Used to detect cycles
    temporarily_marked = set()

    permanently_marked = set()

    def visit(n: Node):
        if n in permanently_marked: return
        if n in temporarily_marked: raise ValueError("Graph contains a cycle")
...
def get_children(tensor: Tensor) -> List[Tensor]:
    if tensor.recipe is None: return []
...
def sorted_computational_graph(tensor: Tensor) -> List[Tensor]:
    ...
    '''
    sorted = topological_sort(tensor, get_children)
...
def backprop(end_node: Tensor, end_grad: Optional[Tensor] = None) -> None:
    ...
    # Get an instance of a class that can look up backward functions for each node type
    ...
    for node in nodes:
        ...
        if node.recipe is None or node.recipe.parents is None: continue

        for (index, parent) in node.recipe.parents.items():
            back_func = BACK_FUNCS.get_back_func(node.recipe.func, index)

            parent_grad = back_func(grads[node], node.array, *node.recipe.args, **node.recipe.kwargs)
            grads[parent] = parent_grad if parent not in grads else grads[parent] + parent_grad
...
def _expand(x: Arr, new_shape) -> Arr:
    ...
    '''
    # take the last len(x.shape) elements of new_shape
    # and replace any -1's with the corresponding elements of new_shape
    # then call np.broadcast_to
    new_shape = list(new_shape)
    for i, s in enumerate(new_shape[-len(x.shape):]):
        if s == -1:
            new_shape[-len(x.shape) + i] = x.shape[i]
...
def sum_back(grad_out: Arr, out: Arr, x: Arr, dim=None, keepdim=False):
    ...
    if dim is None:
        dim = list(range(x.ndim - grad_out.ndim))
...
def maximum_back0(grad_out: Arr, out: Arr, x: Arr, y: Arr):
    ...
    # For elements of x that are equal to y, the gradient is 1/2.
